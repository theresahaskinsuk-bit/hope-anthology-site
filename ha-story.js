(function(){
  ...
  function render(c){
    ...
    const footerCols = (c.footer?.columns||[]).map(col=>`<div class="ha-v3-footer-col"><div class="ha-v3-footer-title">${esc(col.title)}</div>${(col.links||[]).filter(l=>l.label !== 'Instagram' && l.label !== 'Why I sell this way' && l.label !== 'Why we sell this way').map(l=>`<a href="${esc(l.href)}">${esc(l.label)}</a>`).join('')}</div>`).join('');
