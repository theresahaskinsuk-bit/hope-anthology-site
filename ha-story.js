(function(){
  ...
  function render(c){
    ...
    const footerCols = (c.footer?.columns||[]).map(col=>`<div class="ha-v3-footer-col"><div class="ha-v3-footer-title">${esc(col.title)}</div>${(col.links||[]).filter(l=>l.label !== 'Instagram').map(l=>`<a href="${esc(l.href)}">${esc(l.label)}</a>`).join('')}</div>`).join('');
