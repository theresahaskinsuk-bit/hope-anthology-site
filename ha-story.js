(function(){
  ...
  function nutshellHtml(n){
    if (!n) return '';
    return `<section class="ha-story-nutshell"><p class="ha-story-eyebrow twine">${esc(n.eyebrow)}</p><p class="ha-story-nutshell-whisper">${esc(n.whisper)}</p><p class="ha-story-nutshell-body">${esc(n.body)}</p></section>`;
  }
  function sectionHtml(s){
    const dark=s.theme === 'dark';
    const text = `<div class="ha-story-panel-text"><div class="ha-story-num ${dark?'ha-story-num-light':''}">${esc(s.number)}</div><p class="ha-story-eyebrow ${dark?'gold':'twine'}">${esc(s.eyebrow)}</p><h2 class="ha-story-section-title">${s.titleHtml}</h2><p class="ha-story-intro">${esc(s.intro)}</p><blockquote class="ha-story-section-quote">${esc(s.quote)}</blockquote><div class="ha-story-more-prose">${bodyHtml(s)}</div></div>`;
  ...
  function render(c){
    ...
    const nutshell = nutshellHtml(c.nutshell);
    const footerCols = (c.footer?.columns||[]).map(col=>`<div class="ha-v3-footer-col"><div class="ha-v3-footer-title">${esc(col.title)}</div>${(col.links||[]).filter(l=>l.label !== 'Instagram' && l.label !== 'Why I sell this way' && l.label !== 'Why we sell this way').map(l=>`<a href="${esc(l.href)}">${esc(l.label === 'Collaborate' ? 'For Artists' : (l.label === 'My Story' ? 'The Story' : l.label))}</a>`).join('')}${col.title === 'Navigate' ? '<a href="/for-organisations">For organisations</a>' : ''}</div>`).join('');
    root.innerHTML = `<div id="ha-story-v1" class="ha-story-page"><nav class="ha-v3-nav" aria-label="Hope Anthology navigation"><a class="ha-v3-brand" href="/" aria-label="The Hope Anthology home"><img class="ha-v3-logo" src="${asset(c.assets.logo)}" alt=""><span class="ha-v3-sr-only">The Hope Anthology</span></a><button class="ha-v3-menu-toggle" type="button" aria-label="Open menu" aria-controls="ha-story-mobile-menu" aria-expanded="false"><span></span><span></span><span></span></button><div id="ha-story-mobile-menu" class="ha-v3-links">${nav}</div></nav><header class="ha-story-hero"><div class="ha-story-hero-image"><img src="${asset(c.hero.image)}" alt="${esc(c.hero.alt)}"></div><div class="ha-story-hero-text"><p class="ha-story-hero-eyebrow">${esc(c.hero.eyebrow)}</p><h1>${c.hero.titleHtml}</h1><p class="ha-story-hero-subtitle">${esc(c.hero.subtitle)}</p><p class="ha-story-hero-whisper">${esc(c.hero.whisper)}</p><p class="ha-story-scroll">${esc(c.hero.scrollCue)}</p></div></header>${nutshell}<nav class="ha-story-section-nav" aria-label="Story sections">${tabs}</nav>${sections}<section id="${esc(c.finalSection.id)}" class="ha-story-final"><div><p class="ha-story-eyebrow gold">${esc(c.finalSection.eyebrow)}</p><h2>${c.finalSection.titleHtml}</h2><p>${esc(c.finalSection.body)}</p></div><aside><blockquote>${esc(c.finalSection.quote)}</blockquote><p>${esc(c.finalSection.signature)}</p></aside></section><section class="ha-story-collective"><div><p class="ha-story-collective-eyebrow">${esc(c.collective.eyebrow)}</p><h2>${esc(c.collective.title)}</h2><p>${esc(c.collective.body)}</p></div><form class="ha-story-email" onsubmit="event.preventDefault(); this.querySelector('.ha-story-note').textContent='Thank you — mailing list connection can be added when ready.';"><input type="email" placeholder="Email address" aria-label="Email address"><button type="submit">${esc(c.collective.button)}</button><p class="ha-story-note">${esc(c.collective.note)}</p></form></section><footer class="ha-v3-footer"><div class="ha-v3-footer-top"><img class="ha-v3-footer-star" src="${asset(c.assets.footerStar)}" alt="The Hope Anthology botanical star">${footerCols}</div><div class="ha-v3-footer-bottom"><span>${esc(c.footer.copyright)}</span></div></footer></div>`;
