(function(){
  ...
  function productCard(content, item){
    ...
    var action = available && hasUrl(item.etsyUrl)
      ? '<a class="ha-kc-btn ha-kc-btn-keep" href="' + esc(item.etsyUrl) + '" target="_blank" rel="noopener">' + ctaLabel('Get this pattern on Etsy') + '</a>'
  ...
  function groupHtml(content, group){
    var patterns = group.patterns || [];
    return '<section class="ha-make-section-group"><p class="ha-kc-grid-eyebrow">' + esc(group.collection || '') + ' — ' + esc(patterns.length) + (patterns.length === 1 ? ' pattern' : ' patterns') + '</p><div class="ha-kc-card-grid ha-ap-card-grid">' + patterns.map(function(item){ return productCard(content, item); }).join('') + '</div></section>';
  }
